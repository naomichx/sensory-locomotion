...
import mujoco  # make sure this is imported at the top
import math
...
class HalfCheetahEnv(MujocoEnv, utils.EzPickle):
    ...
    * v0: Initial versions release.
    ...
    def step(self, action):
        ...
        # Record velocity and height
        #self.all_vel.append(x_velocity.copy())

        # y coordinate of the middle of the torso
        torso_id = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_BODY, "torso")
        pos = self.data.xpos[torso_id]
        y_pos = pos[2]
        #self.all_y.append(y_pos)

        # y coordinate of the front of the torso
        y_front = self.data.site_xpos[0][2]
        ...
        self.time += 1 # concatenate time
        # truncation=False as the time limit is handled by the `TimeLimit` wrapper added during `make`
        if y_pos < 0.4 or y_pos > 0.9 or y_front < 0.4 or y_front > 0.8: # If the torso is out of this zone, terminate the episode
    ...
    def _get_rew(self, x_velocity: float, action):
        velocity_error = np.abs(self.target_velocity - x_velocity)
        # forward_reward = self._forward_reward_weight*x_velocity
        forward_reward = self._forward_reward_weight * math.exp(- (velocity_error) ** 2 /self.target_velocity)
        #forward_reward = self._forward_reward_weight*  (10-velocity_error**2)
        ctrl_cost = self.control_cost(action)
        #velocity_reward = 0.1* x_velocity
    ...
    def _get_obs(self):
        ...
        if self._exclude_current_positions_from_observation:
            position = position[1:]
        observation = np.concatenate((position, velocity, [np.mod(self.time, 0.2)/0.2])).ravel()
        #observation = np.concatenate((position, velocity, [self.time])).ravel()
